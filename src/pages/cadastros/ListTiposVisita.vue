...
const consultar = (row) => {
	localStorage.setItem('tipovisita', JSON.stringify(row));
	localStorage.setItem('operacao', 'C');
	router.push({ name: 'tipovisitacadastro', params: { operacao: 'C'}})
}
const adicionar = (row) => {
	localStorage.setItem('tipovisita', JSON.stringify(row));
	localStorage.setItem('operacao', 'I');
	router.push({ name: 'tipovisitacadastro', params: { operacao: 'I'}})
}
const editar = (row) => {
	localStorage.setItem('tipovisita', JSON.stringify(row));
	localStorage.setItem('operacao', 'U');
	router.push({ name: 'tipovisitacadastro', params: { operacao: 'U'}})
}
const excluir = (row) => {
	localStorage.setItem('tipovisita', JSON.stringify(row));
	localStorage.setItem('operacao', 'D');
	router.push({ name: 'tipovisitacadastro', params: { operacao: 'D'}})
}
</script>
<script>
export default {
	name: 'listaTiposVisita',
	data() {
		return {
			tiposVisita:[]
		}
